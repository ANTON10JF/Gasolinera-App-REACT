import { Grid, Heading, Box, Image, GridItem, Button, Stack } from "@chakra-ui/react"
    ...
    return (
        <>
            ...
            <Grid textAlign="center" paddingBottom="4rem">
                <Heading marginBottom="40px" size="lg">Resumen del pedido</Heading>
            </Grid>
            <Grid templateColumns="repeat(2, 1fr)" gap={9}>
                {resumeData.map((data, index) => (


                    <GridItem w="100%" borderRadius="10" boxShadow="0px 4px 10px -4px rgb(117 117 177)" padding="25px 15px" key={index}>
                        <Box minHeight="50px">
                            <Heading size="sm">{data.title}</Heading>
                            <Heading size="md" fontWeight="800">{data.result}</Heading>
                        </Box>

                        <Image src={data.image} alt="pruebas" width="100px" margin="0 auto" />
                    </GridItem>
                ))}
            </Grid>
            <Grid marginTop="20px" padding="20px" boxShadow="0px 4px 10px -3px rgb(117 117 117)" borderRadius="10">
